...
public class WheelView extends View {
	...
	public void setViewAdapter(WheelViewAdapter viewAdapter) {
        ...
        if (viewAdapter instanceof AbstractWheelTextAdapter) {
            int textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
                    ((AbstractWheelTextAdapter) viewAdapter).getTextSize(),
                    getContext().getResources().getDisplayMetrics());
            CharSequence itemText = ((AbstractWheelTextAdapter) viewAdapter).getItemText(0);
            textPaint.setTextSize(textSize);
            itemWidth = textPaint.measureText(itemText, 0, itemText.length());
			AbstractWheelTextAdapter adapter = (AbstractWheelTextAdapter) viewAdapter;
			if (adapter.isEnableMultiTextColor()) {
				textPaint.setColor(adapter.getTextSelectedColor());
			} else {
				textPaint.setColor(adapter.getTextColor());
			}
        }
        invalidateWheel(true);
	}
	...
	private Runnable drawCenterRunnable;

	public void hookDrawCenterRect(Runnable runnable) {
		this.drawCenterRunnable = runnable;
	}
	...
	private void drawItems(Canvas canvas) {
		...
		if (adapter != null && adapter.isEnableMultiTextColor()) {
			...
			for (int i = 0; i < itemsLayout.getChildCount(); i++) {
				...
				adapter.setTextViewPadding(view, adapter.getTextPaddingTop(),
						adapter.getTextPaddingBottom());
	...
	private void drawCenterRect(Canvas canvas) {
		...
		if(drawCenterRunnable != null) drawCenterRunnable.run();
	}

	private void setCenterDrawableBounds() {
    ...
    private int hintPadding = 0;

    public void setHint(String hint) {
        this.hint = hint;
    }

    public void setHintSizePx(int sizePx) {
        this.hintSize = sizePx;
        textPaint.setTextSize(sizePx);
    }

    public void setHintPaddingPx(int paddingPx) {
        this.hintPadding = paddingPx;
    }
