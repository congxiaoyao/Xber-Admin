...
/**
 * isLoading变量的线程同步问题
 * 这是一个关于{@link ListLoadablePresenter}接口的实现类
 ...
 * {@link ListLoadablePresenterImpl#pullPagedListData(int)}
 * 只要子类实现这个方法 这里作为父类便可以实现上述三个功能
 * <p>
 * 由于类内持有的view为{@link ListLoadableView}的子类 所以这里可以实现一部分错误处理方法
 * 关于{@link ListLoadablePresenterImpl#isLoading}的使用不是线程安全的 可能会带来一些影响
 * 其他详细注释见方法
 * <p>
 * Created by congxiaoyao on 2016/8/26.
 */
public abstract class ListLoadablePresenterImpl<T extends ListLoadableView>
        extends BasePresenterImpl<T>
        implements ListLoadablePresenter, Action1<Throwable>, Action2<Page, Date> {
    ...
    public void onTimeoutError(SocketTimeoutException exception) {
        super.onTimeoutError(exception);
    ...
    public void onUnknowHostError(UnknownHostException exception) {
        super.onUnknowHostError(exception);
        view.showNetworkError();
    }
